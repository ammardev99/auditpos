class ConfirmationItemModel {
  ...
  factory ConfirmationItemModel.fromJson(Map<String, dynamic> json) {
    double parseDouble(dynamic value) => double.tryParse(value?.toString() ?? '0') ?? 0.0;
    int parseInt(dynamic value) => (double.tryParse(value?.toString() ?? '0') ?? 0.0).round();

    // Determine status from database flags
    final approved = json['is_approved'] == 1 || 
    ...
    return ConfirmationItemModel(
      ...
      systemQty: parseInt(json['sys_qty'] ?? json['system_qty'] ?? json['quantity_instock']),
      physicalQty: parseInt(json['phy_qty'] ?? json['physical_qty'] ?? 0),
      systemPrice: parseDouble(json['sys_price'] ?? json['system_price'] ?? json['current_rate']),
      physicalPrice: parseDouble(json['phy_price'] ?? json['physical_price'] ?? 0.0),
